<script lang="ts">
  ...
  // Teilnehmer-Label (privacy-safe)
  const participantText = $derived(() => {
    const acc = ride.acceptedCount ?? 0;
    const pend = ride.pendingCount ?? 0;
    const total = acc + pend;
    if (total === 0) return null;
    if (acc > 0 && pend > 0) return `${acc} bestätigt · ${pend} anfragend`;
    if (acc > 0) return acc === 1 ? '1 Mitfahrer bestätigt' : `${acc} Mitfahrer bestätigt`;
    return pend === 1 ? '1 Person interessiert' : `${pend} Personen interessiert`;
  });

  // Auslastungs-Prozent für visuellen Balken
  const occupancyPercent = $derived(
    ride.seats > 0 ? Math.round(((ride.seats - ride.seatsAvailable) / ride.seats) * 100) : 0
  );
</script>
...
<div class="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 active:scale-[0.99] transition-transform">
  ...
  <a href="/rides/{ride._id}" class="block">
    <div class="relative h-32 bg-gradient-to-r {gradient} overflow-hidden">
      <img
        src={headerImage}
        alt={ride.eventName}
      ...
      <div class="absolute top-3 right-3 flex flex-col items-end gap-1.5">
        <div class="flex items-center gap-1 bg-black/40 backdrop-blur-sm px-2 py-0.5 rounded-full">
          <span class="text-sm leading-none">{emoji}</span>
          <span class="text-white/90 text-[10px] font-semibold">{label}</span>
        </div>
      ...
      <div class="absolute bottom-0 left-0 right-0 px-3 pb-2.5">
        ...
        <div class="flex items-center gap-1 mt-0.5">
          <svg class="w-2.5 h-2.5 text-white/60 shrink-0" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
          </svg>
          <p class="text-white/60 text-[10px] truncate">{ride.eventLocation}</p>
        </div>
      </div>
    </div>
  </a>
  ...
  <div class="p-3.5 flex flex-col gap-3">
    ...
    <div class="flex items-center justify-between">
      <a href="/users/{ride.driverId}" class="flex items-center gap-2.5 min-w-0">
        {#if ride.driverPhoto}
          <img src={ride.driverPhoto} alt={ride.driverName} class="w-8 h-8 rounded-full object-cover ring-2 ring-gray-100 shrink-0" />
        {:else}
          <div class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bold text-xs ring-2 ring-gray-100 shrink-0">
            {initials(ride.driverName)}
          </div>
        {/if}
        <div class="min-w-0">
          <p class="font-semibold text-gray-900 text-sm leading-none truncate">{ride.driverName}</p>
      ...
      <div class="flex flex-col items-end gap-0.5 shrink-0 ml-2">
        ...
        <div class="flex items-center gap-1">
          <span class="text-xs font-semibold text-gray-500">{ride.seatsAvailable} frei</span>
          {#if ride.seatsAvailable === 0}
            <span class="text-[9px] font-bold bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full">Voll</span>
          {:else if occupancyPercent >= 50}
            <span class="text-[9px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">Fast voll</span>
          {/if}
        </div>
      </div>
    </div>
        ...
        <div class="border-t border-gray-100 pt-1.5 mt-0.5 flex flex-col gap-1.5">
          ...
          {#if preview.detourMinutes > 1}
            <div class="flex items-center justify-between text-[10px] text-gray-400 border-t border-gray-100 pt-1">
              ...
              <span class="{preview.detourMinutes > 20 ? 'text-red-400 font-semibold' : ''}">+{preview.detourMinutes} Min</span>
            </div>
          {/if}
        </div>
      {:else}
    ...
    <div class="flex items-center justify-between pt-0.5">
      <div class="flex items-center gap-1.5">
        <span class="font-bold text-gray-900 text-base">CHF {ride.pricePerPerson.toFixed(2)}</span>
        <span class="text-xs text-gray-400">/ Person</span>
      </div>
      <a
        href={bookHref}
        class="bg-rose-600 text-white px-4 py-2 rounded-full text-xs font-bold hover:bg-rose-700 active:bg-rose-800 transition-colors shadow-sm"
      >
        Mitfahren
      </a>
    </div>
